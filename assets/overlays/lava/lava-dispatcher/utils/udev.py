...
def get_udev_devices(job=None, logger=None, device_info=None, required=False):
    ...
    # NOTE(pbe): pyudev for cdc_ether usb device on YUMi terminals
    #            yields a device node that is 'None'. Processing such
    #            invalid 'device_paths' list further down the pipeline fails.
    #            This patch filters out 'None' values before returning it to
    #            the caller.
    device_paths = [p for p in list(device_paths) if p]
